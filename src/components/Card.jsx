export default function Card({ id, title, img, onSelect, count, selected }) {
    ...
    <div className="relative w-full p-2 outline-none">
      ...
      >
        <div className="isolate w-full hover:shadow-lg transition-all">
          ...
          />
          {count > 1 && (
            <span className="absolute -top-2 right-0 bg-white text-black px-3 py-1 rounded-full text-lg font-semibold">
              {count}
            </span>
          )}
        </div>
        <h4 className=" bg-opacity-50 px-3 py-1 rounded-full text-sm font-semibold">{title}</h4>
      </button>
    </div>
  );
}
